...
$('.yellow-button').click(() => {
    if (order.customer.name && order.customer.phoneNumber) {
        ...
        if (!order.customer.name) {
            $('#nameError').html('Your name is required to place an order').removeClass('hidden')
        }
        if (!order.customer.phoneNumber) {
            $('#phoneError').html('Your phone number is required to place an order').removeClass('hidden')
        }
    }
})